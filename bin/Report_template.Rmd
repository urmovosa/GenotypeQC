...
params:
  ...
  SD: 3
  data_type: "array"
  imputation_filter_enabled: false
  vcf_maf: 0.01
  vcf_hwe: 0.000001
  vcf_imp: 0.8
  genotype_field: ""
...
Input data type: **`r params$data_type`**.

`r if (params$data_type == "imputed") paste0("Final VCF filters: MAF >= ", params$vcf_maf, "; HWE >= ", params$vcf_hwe, if (isTRUE(params$imputation_filter_enabled)) paste0("; ", params$imputation_metric_field, " >= ", params$vcf_imp) else "; imputation quality filter disabled", ".") else if (params$data_type == "wgs") paste0("Final VCF filters: MAF >= ", params$vcf_maf, "; HWE >= ", params$vcf_hwe, ".") else "Final VCF filtering is not produced for PLINK array input."`
...
overview_table[overview_table["Stage"] == "Raw file", "Stage"] <- "Input variants"
...
if (nrow(before_counts) == 0 || nrow(after_counts) == 0) {
  ...
  if (combined_before > 0) {
    cat(sprintf("Variant retention after final filters: **%.2f%%**.\n\n",
                100 * combined_after / combined_before))
  }
...
`r if (params$data_type == "wgs") "### WGS variant composition" else ""`

```{r, message = FALSE, warning = FALSE, echo = FALSE, results = 'asis', eval = params$data_type == "wgs"}
prefilter_stats_files <- list.files(vcf_metrics_dir, pattern = "_prefilter\\.stats\\.txt$", full.names = TRUE)
filtered_stats_files <- list.files(vcf_metrics_dir, pattern = "_filtered\\.stats\\.txt$", full.names = TRUE)

summarise_bcftools_stats <- function(files, stage_label) {
  totals <- c(SNPs = 0, Indels = 0, Other = 0, Transitions = 0, Transversions = 0)
  for (filepath in files) {
    lines <- readLines(filepath, warn = FALSE)
    sn_lines <- strsplit(grep("^SN\\t", lines, value = TRUE), "\\t")
    get_sn <- function(key) {
      matches <- vapply(sn_lines, function(fields) length(fields) >= 4 && fields[3] == key, logical(1))
      if (!any(matches)) return(0)
      sum(as.numeric(vapply(sn_lines[matches], `[[`, character(1), 4)), na.rm = TRUE)
    }
    totals["SNPs"] <- totals["SNPs"] + get_sn("number of SNPs:")
    totals["Indels"] <- totals["Indels"] + get_sn("number of indels:")
    totals["Other"] <- totals["Other"] + get_sn("number of others:")

    tstv <- strsplit(grep("^TSTV\\t", lines, value = TRUE), "\\t")
    if (length(tstv) > 0 && length(tstv[[1]]) >= 4) {
      totals["Transitions"] <- totals["Transitions"] + as.numeric(tstv[[1]][3])
      totals["Transversions"] <- totals["Transversions"] + as.numeric(tstv[[1]][4])
    }
  }
  ...
  data.frame(
    Stage = stage_label,
    SNPs = totals["SNPs"],
    Indels = totals["Indels"],
    Other = totals["Other"],
    Ti_Tv = if (totals["Transversions"] > 0) totals["Transitions"] / totals["Transversions"] else NA_real_,
    row.names = NULL
  )
}

if (length(prefilter_stats_files) == 0 || length(filtered_stats_files) == 0) {
  cat("WGS variant composition is unavailable because per-chromosome bcftools stats files were not found.\n")
} else {
  wgs_stats <- rbind(
    summarise_bcftools_stats(prefilter_stats_files, "Before final filters"),
    summarise_bcftools_stats(filtered_stats_files, "After final filters")
  )
  knitr::kable(wgs_stats, digits = 3)
}
```

`r if (params$data_type == "imputed") "### Genotyped and imputed variant counts before QC filters (diagnostic)" else ""`

```{r, message = FALSE, warning = FALSE, echo = FALSE, results = 'asis', eval = params$data_type == "imputed"}
...
```{r, fig.height = 4.5, fig.width = 10, fig.align = "center", message = FALSE, warning = FALSE, echo = FALSE, eval = params$data_type == "imputed" && exists("typed_imputed_plot_ready") && typed_imputed_plot_ready}
...
`r if (params$data_type == "imputed") "**Per-chromosome genotyped vs imputed variant counts before QC filters. Plot shown when at least one usable genotyped/imputed indicator INFO field is available (including optional configured field).**" else ""`
...
`r if (params$data_type == "imputed") "### Imputation quality distributions (genome-wide)" else ""`
...
impq_long$Stage <- factor(impq_long$Stage, levels = c("Before QC filters", "After QC filters"))
...
```{r, fig.height = 4.5, fig.width = 10, fig.align = "center", message = FALSE, warning = FALSE, echo = FALSE, eval = params$data_type == "imputed" && exists("impq_plot_ready") && impq_plot_ready}
...
`r if (params$data_type == "imputed") "**Genome-wide imputation quality histograms before and after QC filters.**" else ""`

`r if (params$data_type == "imputed") "Interpretation: for well-imputed data, this distribution should be **right-heavy** (most variants concentrated toward high imputation-quality values, e.g. near 1 for R2-style metrics)." else ""`
